...
class Arena:
    ...
    def __init__(self,
        ...
        """
        assert resol%2==1, (
            "'resol' needs to be an odd number to allocate anchors on the center of walls."
        )
        self.resol = resol
        ...
        for i in range(self.resol):
            ...
            for j in range(6):
                ...
                theta += 2*np.pi/3
                for k in range(i):
                    if i==self.resol-1 and k==0:
                        self.corner_idxs.append(len(anchors))
                    if i==self.resol-1 and j%2==1 and k==self.resol//2:
                        self.box_idxs.append(len(anchors))
                    x = x0+k/(self.resol-1)*np.cos(theta)
                    y = y0+k/(self.resol-1)*np.sin(theta)
                    anchors.append([x, y])
        ...
        for j in range(6):
            ...
            for i in range(self.resol):
                if i==0:
                    ...
                    x0, y0 = r*np.cos(theta), r*np.sin(theta)
                    for k in range(i):
                        x = x0+k/(self.resol-1)*np.cos(theta+2*np.pi/3)
                        y = y0+k/(self.resol-1)*np.sin(theta+2*np.pi/3)
